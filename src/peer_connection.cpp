...
namespace srtc {
    ...
    PeerConnection::PeerConnection(Direction direction)
            : mDirection(direction), mEventLoop(EventLoop::factory())
#ifdef NDEBUG
#else
    ...
    std::pair<std::shared_ptr<SdpOffer>, Error> PeerConnection::createPublishOffer(
        ...
        if (mDirection != Direction::Publish) {
            return {{},
                    {Error::Code::InvalidData, "The peer connection's direction is not publish"}};
        }
    ...
    std::pair<std::shared_ptr<SdpOffer>, Error> PeerConnection::createSubscribeOffer(
        ...
        if (mDirection != Direction::Subscribe) {
            return {{},
                    {Error::Code::InvalidData, "The peer connection's direction is not subscribe"}};
        }
    ...
    std::pair<std::shared_ptr<SdpAnswer>, Error> PeerConnection::parsePublishAnswer(
            const std::shared_ptr<SdpOffer> &offer, const std::string &answer,
    ...
    Error
    PeerConnection::setVideoSimulcastCodecSpecificData(const std::string &layerName, std::vector<ByteBuffer> &&list) {
        if (mDirection != Direction::Publish) {
            return {Error::Code::InvalidData, "The peer connection's direction is not publish"};
        }
            ...
            if (rtt.has_value()) {
                ...
                if (const auto track = mVideoSingleTrack) {
                    ...
                    if (error.isError()) {
                        ...
                        mJitterBufferVideo = std::make_shared<JitterBuffer>(track, depacketizer, kJitterBufferSize,
                                                                            length, nackDelay);
                    }
                }
                if (const auto track = mAudioTrack) {
                    ...
                    if (error.isError()) {
                        ...
                        mJitterBufferAudio = std::make_shared<JitterBuffer>(track, depacketizer, kJitterBufferSize,
                                                                            length, nackDelay);
                    }
                }
            }
    ...
    void
    PeerConnection::onCandidateReceivedMediaPacket(PeerCandidate *candiate, const std::shared_ptr<RtpPacket> &packet) {
#ifdef NDEBUG
#else
        const auto config = mSdpOffer->getConfig();
    ...
    void PeerConnection::sendPictureLossIndicator() {
        ...
        mTaskPictureLossIndicator =
                mLoopScheduler->submit(std::chrono::milliseconds(config.pli_interval_millis), __FILE__, __LINE__,
                                       [this] {
                                           sendPictureLossIndicator();
                                       });
