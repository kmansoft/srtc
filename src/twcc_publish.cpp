                                              ...
                                              size_t encryptedSize)
{
    ...
    if (!mPacketList) {
        mMinSeq = mMaxSeq = seq;
        mPacketList = new PublishPacket[kMaxPacketCount];
        std::memset(mPacketList, 0, sizeof(PublishPacket) * kMaxPacketCount);
        curr = mPacketList + (seq & kMaxPacketMask);
    } else {
        while (true) {
            ...
            mMaxSeq += 1;
            curr = mPacketList + (mMaxSeq & kMaxPacketMask);
...
PublishPacket* PublishPacketHistory::get(uint16_t seq) const
{
    ...
    if (mMinSeq <= mMaxSeq) {
        if (mMinSeq <= seq && seq <= mMaxSeq) {
            return mPacketList + (seq & kMaxPacketMask);
        }
    } else {
...
void PublishPacketHistory::update()
{
...
bool PublishPacketHistory::calculateBandwidthTrend(int64_t now, PublishPacket* max)
{
    ...
    for (uint16_t curr_seq = max->seq;;) {
        const auto curr_ptr = mPacketList + (curr_seq & kMaxPacketMask);

        if (curr_ptr->received_time_present) {

            if (curr_seq != mMinSeq) {
                const uint16_t prev_seq = curr_seq - 1;
                const auto prev_ptr = mPacketList + (prev_seq & kMaxPacketMask);
