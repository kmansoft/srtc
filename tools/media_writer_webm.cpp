...
void MediaWriterWebm::writeWebM()
{
    // Calculate duration
    ...
    writeEBMLElement(mFile, segment_id, segment_content.data(), segment_data_size);
}

void MediaWriterWebm::writeEBMLHeader()
{
    ...
    writeEBMLElement(mFile, ebml_header_id, header_buffer.data(), header_buffer.size());
}

void MediaWriterWebm::writeSegmentInfo(FILE* file, uint64_t duration_ns)
{
    // Info (0x1549A966)
...
void MediaWriterWebm::writeTracks(FILE* file)
{
    // Tracks (0x1654AE6B)
    ...
    uint16_t frame_width = mFrameWidth;
    uint16_t frame_height = mFrameHeight;
...
void MediaWriterWebm::writeClusters(FILE* file)
{
    if (mFrameList.empty())
        return;
...
void MediaWriterWebm::writeVarInt(FILE* file, uint64_t value)
{
